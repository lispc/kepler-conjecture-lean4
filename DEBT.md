...
> `Kepler.Assembly.the_kepler_conjecture_from_interfaces` 的 `#print axioms`（探针 2026-09-21 重跑）。只统计**装配后主定理实际依赖**的公理：标准三公理 + sorryAx + 特许 native_decide shard 族（DECISIONS.md 2026-08-10 scoped exception）+ 无异常项。

`sorryAx` 来源（2026-09-21 方向 A/B/C/D 深挖后）——骨架已从 3 枚粗接口展开为**分层可达债务图**：
1. **粗接口 2 枚**：`nonlinearInequalities`（内含主叶子 `main_nonlinear_terminal_v11`，G4 汇合点，对接规格 docs/mnt11-interface.md）、`lpArchiveCertificates`（逐图 LP 证书总库，Phase 3 持久化填实；桥章 TameLp.lean 已落锚）；
2. **TameSpine 13 枚骨架占位**（tame 文字章 capstone，Assembly §2c）；
3. **章节出口 HOLD**：LocalConcl 28/90、PackingConcl 9/62（逐条原因见文件内 ledger）；
4. **已真化**（不再计债）：`textCapstone`、`linearProgrammingResults`（纯逻辑推导）、`assembly`、Concl 层 115 条 WIRE。
