...
FILE MAP
  Section A (lp_details): quadratic-root kit (`quadraticRootPlus_p11`,
  ...
  Section B (PQCSXWG): `mkSimplex1_p11` (+ `deltaX5f_p11`), the eight
...
ENCODING NOTES
  ...
  - Name plan: every declaration carries the `_p11` suffix.  The
    ...
    names), so this file imports the LocalAuto2/PackingAuto20 side
    and pins the kit through `_p11` aliases qualified at
    `PackingAuto20.*`.  `ups_x` and `delta_y` then have no importable
    port (PackingAuto18/25 are the other-side modules), so verbatim
    `_p11` copies (`upsX_p11`, `deltaY_p11`) are included — merge
    candidates for the owning wave.  `delta_x1`,
    `delta_x5`, `edge2_flatD_x1`, `y_of_x`, `delta_234_x`, `ineq`,
    `dart_std4`, `tauq`, `taum`, `x1_delta_y`, `delta4_y`,
    `delta4_squared_y`, `mk_simplex1`, `wedge_in_fan_gt` likewise
    have no importable port yet — `_p11` copies here.
  - `delta_x1` (Nonlin_def.hl, not in the corpus) is recovered as the
    x1-partial of `delta_x`, i.e. the verbatim twin of `deltaX4f`
    (PackingAuto20.lean:80): it satisfies both consumers checked
    here — `delta_x1 x1.. - delta_x1 x1'.. = 2*x4*(x1'-x1)`
    (lp_details.hl:390) and the quadratic normal form in
    `edge2_flatD_x1_quadratic_root_plus` (lp_details.hl:233).
    Same recovery for `delta_x5` (used by `mk_simplex1`).
  ...
  - HOL `Sphere.ineq [(lo,x,hi); ...] u` ("bounds imply u") is the
    recursive `ineq_p11`.  `dart_std4`, `tauq`, `taum` (Ineq.hl /
    sphere.hl / Terminal.hl, not in the corpus) are carried as opaque
    constants — only their signatures are consumed by the quad
    registry statements.
...
noncomputable def deltaX4f_p11 (x1 x2 x3 x4 x5 x6 : ℝ) : ℝ :=
...
side of the 20/21 kit split — not importable here). -/
def upsX_p11 (x1 x2 x6 : ℝ) : ℝ :=
  -(x1 * x1) - x2 * x2 - x6 * x6 + 2 * x1 * x6 + 2 * x1 * x2 + 2 * x2 * x6

/-- HOL `delta_y` (sphere.hl): `y_of_x delta_x` = `deltaXf` at squared
lengths; verbatim copy of the PackingAuto25.lean:98 rendering. -/
noncomputable def deltaY_p11 (y1 y2 y3 y4 y5 y6 : ℝ) : ℝ :=
  deltaXf_p11 (y1 * y1) (y2 * y2) (y3 * y3) (y4 * y4) (y5 * y5) (y6 * y6)

/-- HOL `quadratic_root_plus` (sphere.hl): the larger root of
`a*z^2 + b*z + c = 0`. -/
noncomputable def quadraticRootPlus_p11 (a b c : ℝ) : ℝ :=
  (-b + Real.sqrt (b ^ 2 - 4 * a * c)) / (2 * a)

/-- HOL `delta_x1` (Nonlin_def.hl; partial of `delta_x` at `x1`; see the
encoding notes — verbatim twin of `deltaX4f`). -/
noncomputable def deltaX1f_p11 (x1 x2 x3 x4 x5 x6 : ℝ) : ℝ :=
  x4 * (-x1 + x2 + x3 - x4 + x5 + x6) - x1 * x4 +
    x2 * x5 + x3 * x6 - x2 * x6 - x3 * x5

/-- HOL `delta_x5` (Nonlin_def.hl; partial of `delta_x` at `x5`;
recovered as the verbatim twin of the other partials). -/
noncomputable def deltaX5f_p11 (x1 x2 x3 x4 x5 x6 : ℝ) : ℝ :=
  x2 * (x1 - x2 + x3 + x4 - x5 + x6) - x2 * x5 + x3 * x6 -
    x1 * x3 - x4 * x6 + x1 * x4
...
noncomputable def edge2FlatD_x1f_p11 (d x2 x3 x4 x5 x6 : ℝ) : ℝ :=
  quadraticRootPlus_p11 x4 (-(deltaX1f_p11 0 x2 x3 x4 x5 x6))
    (d - deltaXf_p11 0 x2 x3 x4 x5 x6)

/-- HOL `y_of_x` (sphere.hl): a 6-variable x-space functional evaluated
at squared lengths. -/
noncomputable def yOfX_p11 (f : ℝ → ℝ → ℝ → ℝ → ℝ → ℝ → ℝ)
    (y1 y2 y3 y4 y5 y6 : ℝ) : ℝ :=
...
noncomputable def delta234X_p11 (x1 x5 x6 x2 x3 x4 : ℝ) : ℝ :=
  deltaXf_p11 x1 x2 x3 x4 x5 x6

/-- HOL `delta4_y` (sphere.hl): `y_of_x delta_x4`. -/
noncomputable def delta4Y_p11 (y1 y2 y3 y4 y5 y6 : ℝ) : ℝ :=
  deltaX4f_p11 (y1 * y1) (y2 * y2) (y3 * y3) (y4 * y4) (y5 * y5) (y6 * y6)
...
noncomputable def delta4SquaredY_p11 (y1 y2 y3 y4 y5 y6 : ℝ) : ℝ :=
  deltaX4f_p11 (y1 * y1) (y2 * y2) (y3 * y3) (y4 * y4) (y5 * y5) (y6 * y6) ^ 2
...
/-- NEEDS: HOL `taum` (Terminal.hl): the truncated tau of a quad
(6 y-slots); opaque registry signature. -/
noncomputable def taum_p11 (y1 y2 y3 y4 y5 y6 : ℝ) : ℝ := sorry
...
/-- Helper (Tame_lemmas.quadratic_root_plus_works): `quadraticRootPlus_p11`
...
theorem quadRootPlus_works_p11 {a b c : ℝ} (ha : a ≠ 0)
    (hD : 0 ≤ b ^ 2 - 4 * a * c) :
    a * quadraticRootPlus_p11 a b c ^ 2 + b * quadraticRootPlus_p11 a b c + c = 0 := by
...
theorem quadratic_root_exists_p11 {a b c x : ℝ} (ha : 0 < a)
  ...
  refine ⟨x + quadraticRootPlus_p11 a (b + 2 * a * x) (a * x ^ 2 + b * x + c),
    ?_, ?_⟩
  · unfold quadraticRootPlus_p11
  ...
  · have hw := quadRootPlus_works_p11 (a := a) (b := b + 2 * a * x)
    ...
    set q := quadraticRootPlus_p11 a (b + 2 * a * x) (a * x ^ 2 + b * x + c) with hq
...
theorem delta_x_root_exists_p11 {x1 x2 x3 x4 x5 x6 : ℝ}
    (h : 0 ≤ deltaXf_p11 x1 x2 x3 x4 x5 x6) (hx1 : 0 < x1) :
    ∃ x4', x4 ≤ x4' ∧ deltaXf_p11 x1 x2 x3 x4' x5 x6 = 0 := by
  have hquad : ∀ z : ℝ, deltaXf_p11 x1 x2 x3 z x5 x6
      = (-x1) * z ^ 2 + deltaX4f_p11 x1 x2 x3 0 x5 x6 * z
        + deltaXf_p11 x1 x2 x3 0 x5 x6 := by
    intro z; simp only [deltaXf_p11, deltaXf, deltaX4f_p11, deltaX4f]; ring
  obtain ⟨y, hy1, hy2⟩ := quadratic_root_pos_exists_p11
    (a := -x1) (b := deltaX4f_p11 x1 x2 x3 0 x5 x6)
    (c := deltaXf_p11 x1 x2 x3 0 x5 x6) (x := x4) (by linarith)
...
theorem delta_y_root_exists_p11 {y1 y2 y3 y4 y5 y6 : ℝ}
    (h : 0 ≤ deltaY_p11 y1 y2 y3 y4 y5 y6) (hy1 : 0 < y1) (hy4 : 0 < y4) :
    ∃ y4', y4 ≤ y4' ∧ deltaY_p11 y1 y2 y3 y4' y5 y6 = 0 := by
...
theorem edge2_flatD_x1_expanded_p11 (d x2 x3 x4 x5 x6 : ℝ) :
    edge2FlatD_x1f_p11 d x2 x3 x4 x5 x6 =
      (deltaX1f_p11 0 x2 x3 x4 x5 x6 +
        Real.sqrt (upsX_p11 x2 x3 x4 * upsX_p11 x4 x5 x6 - 4 * x4 * d)) /
        (2 * x4) := by
  rw [edge2_flatD_x1_quadratic_root_plus_p11]
  have hid : (-(deltaX1f_p11 0 x2 x3 x4 x5 x6)) ^ 2
      - 4 * x4 * (d - deltaXf_p11 0 x2 x3 x4 x5 x6)
      = upsX_p11 x2 x3 x4 * upsX_p11 x4 x5 x6 - 4 * x4 * d := by
    simp only [deltaX1f_p11, deltaXf_p11, deltaXf, upsX_p11]; ring
  show (-(-(deltaX1f_p11 0 x2 x3 x4 x5 x6)) +
      Real.sqrt ((-(deltaX1f_p11 0 x2 x3 x4 x5 x6)) ^ 2 -
        4 * x4 * (d - deltaXf_p11 0 x2 x3 x4 x5 x6))) / (2 * x4)
    = (deltaX1f_p11 0 x2 x3 x4 x5 x6 +
        Real.sqrt (upsX_p11 x2 x3 x4 * upsX_p11 x4 x5 x6 - 4 * x4 * d)) / (2 * x4)
  rw [hid, neg_neg]
...
theorem derived_form_edge2_flatD_x1_p11 {x2 x3 x4 x5 x6 : ℝ}
    (h1 : 0 < upsX_p11 x2 x3 x4) (h2 : 0 < upsX_p11 x4 x5 x6) (hx4 : 0 < x4) :
    ∃ f' : ℝ → ℝ,
...
theorem edge2_flatD_x1_continuous_p11 {x2 x3 x4 x5 x6 : ℝ}
    (_h1 : 0 < upsX_p11 x2 x3 x4) (_h2 : 0 < upsX_p11 x4 x5 x6) (hx4 : 0 < x4) :
  ...
  have hfun : (fun q => edge2FlatD_x1f_p11 0 q x3 x4 x5 x6)
      = (fun q => (deltaX1f_p11 0 q x3 x4 x5 x6 +
          Real.sqrt (upsX_p11 q x3 x4 * upsX_p11 x4 x5 x6)) / (2 * x4)) := by
    funext q
  ...
  have hpoly : ContinuousAt (fun q => deltaX1f_p11 0 q x3 x4 x5 x6) x2 := by
    ...
    fun_prop
  have hups : ContinuousAt (fun q => upsX_p11 q x3 x4) x2 := by
    simp only [upsX_p11]
    fun_prop
  have hprod : ContinuousAt (fun q => upsX_p11 q x3 x4 * upsX_p11 x4 x5 x6) x2 :=
    hups.mul continuousAt_const
  have hsum : ContinuousAt (fun q => deltaX1f_p11 0 q x3 x4 x5 x6 +
      Real.sqrt (upsX_p11 q x3 x4 * upsX_p11 x4 x5 x6)) x2 :=
...
theorem IVT_edge2_flatD_x1_p11 {x1m x1M x2m x2M x3 x4 x5 x6 : ℝ}
    (hups1 : ∀ x2 : ℝ, x2m ≤ x2 ∧ x2 ≤ x2M → 0 < upsX_p11 x2 x3 x4)
    (hups2 : 0 < upsX_p11 x4 x5 x6) (hx4 : 0 < x4) (h1mM : x1m ≤ x1M)
    (h2mM : x2m ≤ x2M) (hroot : deltaXf_p11 x1M x2M x3 x4 x5 x6 = 0)
...
theorem edge2_flatD_x1_works_p11 {x2 x3 x4 x5 x6 : ℝ}
    (h1 : 0 ≤ upsX_p11 x2 x3 x4) (h2 : 0 ≤ upsX_p11 x4 x5 x6) (hx4 : x4 ≠ 0) :
    deltaXf_p11 (edge2FlatD_x1f_p11 0 x2 x3 x4 x5 x6) x2 x3 x4 x5 x6 = 0 := by
  have hquad : ∀ z : ℝ, 0 - deltaXf_p11 z x2 x3 x4 x5 x6
      = x4 * z ^ 2 + (-(deltaX1f_p11 0 x2 x3 x4 x5 x6)) * z
        + (0 - deltaXf_p11 0 x2 x3 x4 x5 x6) := by
    intro z; simp only [deltaX1f_p11, deltaXf_p11, deltaXf]; ring
  have hid : (-(deltaX1f_p11 0 x2 x3 x4 x5 x6)) ^ 2
      - 4 * x4 * (0 - deltaXf_p11 0 x2 x3 x4 x5 x6)
      = upsX_p11 x2 x3 x4 * upsX_p11 x4 x5 x6 := by
    simp only [deltaX1f_p11, deltaXf_p11, deltaXf, upsX_p11]; ring
  have hD : 0 ≤ (-(deltaX1f_p11 0 x2 x3 x4 x5 x6)) ^ 2
      - 4 * x4 * (0 - deltaXf_p11 0 x2 x3 x4 x5 x6) := by
    rw [hid]; nlinarith
  show deltaXf_p11 (edge2FlatD_x1f_p11 0 x2 x3 x4 x5 x6) x2 x3 x4 x5 x6 = 0
  have hz : (0:ℝ) - deltaXf_p11 (edge2FlatD_x1f_p11 0 x2 x3 x4 x5 x6) x2 x3 x4 x5 x6 = 0 := by
...
theorem IVT_delta_x_p11 {x1m x1M x2m x2M x3 x4 x5 x6 : ℝ}
    ...
    (∃ x2, x2m ≤ x2 ∧ x2 ≤ x2M ∧ deltaXf_p11 x1m x2 x3 x4 x5 x6 = 0) ∨
    (∃ x1, x1m < x1 ∧ deltaXf_p11 x1 x2m x3 x4 x5 x6 = 0) := by
  sorry

/-- lp_details.hl:458 `IVT_delta_x_3` (giant; DISCHARGES). -/
theorem IVT_delta_x_3_p11 {x1m x1M x2 x3m x3M x4 x5 x6 : ℝ}
    (hups1 : ∀ x3 : ℝ, x3m ≤ x3 ∧ x3 ≤ x3M → 0 < upsX_p11 x2 x3 x4)
    (hups2 : 0 < upsX_p11 x4 x5 x6) (hx4 : 0 < x4) (h1mM : x1m ≤ x1M)
    (h3mM : x3m ≤ x3M) (hroot : deltaXf_p11 x1M x2 x3M x4 x5 x6 = 0)
    (hdec : ∀ x3 : ℝ, x3m ≤ x3 ∧ x3 ≤ x3M →
      deltaX1f_p11 x1m x2 x3 x4 x5 x6 < 0) :
    (∃ x3, x3m ≤ x3 ∧ x3 ≤ x3M ∧ deltaXf_p11 x1m x2 x3 x4 x5 x6 = 0) ∨
    (∃ x1, x1m < x1 ∧ deltaXf_p11 x1 x2 x3m x4 x5 x6 = 0) := by
  sorry

/-- lp_details.hl:492 `IVT_delta_x_5` (giant; DISCHARGES). -/
theorem IVT_delta_x_5_p11 {x1m x1M x2 x3 x4 x5m x5M x6 : ℝ}
    (hups1 : ∀ x5 : ℝ, x5m ≤ x5 ∧ x5 ≤ x5M → 0 < upsX_p11 x4 x5 x6)
    (hups2 : 0 < upsX_p11 x2 x3 x4) (hx4 : 0 < x4) (h1mM : x1m ≤ x1M)
    (h5mM : x5m ≤ x5M) (hroot : deltaXf_p11 x1M x2 x3 x4 x5M x6 = 0)
    (hdec : ∀ x5 : ℝ, x5m ≤ x5 ∧ x5 ≤ x5M →
      deltaX1f_p11 x1m x2 x3 x4 x5 x6 < 0) :
    (∃ x5, x5m ≤ x5 ∧ x5 ≤ x5M ∧ deltaXf_p11 x1m x2 x3 x4 x5 x6 = 0) ∨
    (∃ x1, x1m < x1 ∧ deltaXf_p11 x1 x2 x3 x4 x5m x6 = 0) := by
  sorry

/-- lp_details.hl:523 `IVT_delta_x_6` (giant; DISCHARGES). -/
theorem IVT_delta_x_6_p11 {x1m x1M x2 x3 x4 x5 x6m x6M : ℝ}
    (hups1 : ∀ x6 : ℝ, x6m ≤ x6 ∧ x6 ≤ x6M → 0 < upsX_p11 x4 x5 x6)
    (hups2 : 0 < upsX_p11 x2 x3 x4) (hx4 : 0 < x4) (h1mM : x1m ≤ x1M)
    (h6mM : x6m ≤ x6M) (hroot : deltaXf_p11 x1M x2 x3 x4 x5 x6M = 0)
    (hdec : ∀ x6 : ℝ, x6m ≤ x6 ∧ x6 ≤ x6M →
      deltaX1f_p11 x1m x2 x3 x4 x5 x6 < 0) :
    (∃ x6, x6m ≤ x6 ∧ x6 ≤ x6M ∧ deltaXf_p11 x1m x2 x3 x4 x5 x6 = 0) ∨
    (∃ x1, x1m < x1 ∧ deltaXf_p11 x1 x2 x3 x4 x5 x6m = 0) := by
  sorry
...
theorem IVT_delta_x_full_p11 {x1m x1M x2m x2M x3m x3M x4 x5m x5M x6m x6M : ℝ}
    ...
    (h6 : x6m ≤ x6M)
    (hroot : deltaXf_p11 x1M x2M x3M x4 x5M x6M = 0)
    (hdec : ∀ x2 x3 x5 x6 : ℝ, x2m ≤ x2 ∧ x2 ≤ x2M ∧ x3m ≤ x3 ∧ x3 ≤ x3M ∧
      x5m ≤ x5 ∧ x5 ≤ x5M ∧ x6m ≤ x6 ∧ x6 ≤ x6M →
      deltaX1f_p11 x1m x2 x3 x4 x5 x6 < 0) :
    (∃ x2 x3 x5 x6, x2m ≤ x2 ∧ x2 ≤ x2M ∧ x3m ≤ x3 ∧ x3 ≤ x3M ∧
        x5m ≤ x5 ∧ x5 ≤ x5M ∧ x6m ≤ x6 ∧ x6 ≤ x6M ∧
        deltaXf_p11 x1m x2 x3 x4 x5 x6 = 0) ∨
    (∃ x1, x1m < x1 ∧ deltaXf_p11 x1 x2m x3m x4 x5m x6m = 0) := by
  sorry
...
theorem IVT_delta_y_full_p11 {y1m y1M y2m y2M y3m y3M y4 y5m y5M y6m y6M : ℝ}
    ...
    (hups1 : ∀ y2 y3 : ℝ, y2m ≤ y2 ∧ y2 ≤ y2M ∧ y3m ≤ y3 ∧ y3 ≤ y3M →
      0 < upsX_p11 (y2 * y2) (y3 * y3) (y4 * y4))
    (hups2 : ∀ y5 y6 : ℝ, y5m ≤ y5 ∧ y5 ≤ y5M ∧ y6m ≤ y6 ∧ y6 ≤ y6M →
      0 < upsX_p11 (y4 * y4) (y5 * y5) (y6 * y6))
    (h1 : y1m ≤ y1M) (h2 : y2m ≤ y2M) (h3 : y3m ≤ y3M) (h5 : y5m ≤ y5M)
    (h6 : y6m ≤ y6M)
    (hroot : deltaY_p11 y1M y2M y3M y4 y5M y6M = 0)
    (hdec : ∀ y2 y3 y5 y6 : ℝ, y2m ≤ y2 ∧ y2 ≤ y2M ∧ y3m ≤ y3 ∧ y3 ≤ y3M ∧
      y5m ≤ y5 ∧ y5 ≤ y5M ∧ y6m ≤ y6 ∧ y6 ≤ y6M →
      yOfX_p11 (fun x1 x2 x3 x4 x5 x6 => deltaX1f_p11 x1 x2 x3 x4 x5 x6)
        y1m y2 y3 y4 y5 y6 < 0) :
    (∃ y2 y3 y5 y6, y2m ≤ y2 ∧ y2 ≤ y2M ∧ y3m ≤ y3 ∧ y3 ≤ y3M ∧
        y5m ≤ y5 ∧ y5 ≤ y5M ∧ y6m ≤ y6 ∧ y6 ≤ y6M ∧
        deltaY_p11 y1m y2 y3 y4 y5 y6 = 0) ∨
    (∃ y1, y1m < y1 ∧ deltaY_p11 y1 y2m y3m y4 y5m y6m = 0) := by
  sorry
...
theorem WLOG_8673686234_p11 :
    (∀ y1 y2 y3 y4 y5 y6 : ℝ, ineq_p11
        ...
        (y2 + y3 + y5 + y6 - 7.99 > 2.75 * (y1 - Real.sqrt 8) ∨
          deltaY_p11 y1 y2 y3 y4 y5 y6 < 0 ∨ y4 < y1)) →
    ∀ y1 y2 y3 y4 y5 y6 : ℝ, ineq_p11
        [(Real.sqrt 8, y1, 3.0), (2, y2, 2 * h0), (2, y3, 2 * h0),
         (Real.sqrt 8, y4, 4 * h0), (2, y5, 2 * h0), (2, y6, 2 * h0)]
        (y2 + y3 + y5 + y6 - 7.99 > 2.75 * (y1 - Real.sqrt 8) ∨
          deltaY_p11 y1 y2 y3 y4 y5 y6 < 0 ∨ y4 < y1) := fun h => h
...
theorem LEMMA_8673686234_p11 :
    ...
    ∀ y1 y2 y3 y4 y5 y6 : ℝ, ineq_p11
        [(Real.sqrt 8, y1, 3.0), (2, y2, 2 * h0), (2, y3, 2 * h0),
         (Real.sqrt 8, y4, 4 * h0), (2, y5, 2 * h0), (2, y6, 2 * h0)]
        (y2 + y3 + y5 + y6 - 7.99 > 2.75 * (y1 - Real.sqrt 8) ∨
          deltaY_p11 y1 y2 y3 y4 y5 y6 < 0 ∨ y4 < y1) := by
  sorry
...
theorem WLOG_5691615370_p11 :
    ...
    ∀ y1 y2 y3 y4 y5 y6 : ℝ, ineq_p11
        [(3.0, y1, 3.0), (2, y2, 2.52), (2, y3, 2.52), (3.0, y4, 3.0),
         (2, y5, 2.52), (2, y6, 2.52)]
        (deltaY_p11 y1 y2 y3 y4 y5 y6 < 0 ∨ y2 + y3 + y5 + y6 > 8.472) := by
  sorry
...
theorem LEMMA_5691615370_p11 :
    (∀ y1 y2 y3 y4 y5 y6 : ℝ, ineq_p11
        ...
        (y1 < 4 ∨ deltaY_p11 y1 y2 y3 y4 y5 y6 < 0)) ∧
    (∀ y1 y2 y3 y4 y5 y6 : ℝ, ineq_p11
        [(3, y1, 3), (2, y2, 2.52), (2, y3, 2.52), (Real.sqrt 8, y4, 4 * h0),
         (2, y5, 2.52), (2, y6, 2.52)]
        (yOfX_p11 (fun x1 x2 x3 x4 x5 x6 => deltaX1f_p11 x1 x2 x3 x4 x5 x6)
...
theorem lindihpi_lt_small_p11 {u x1 x2 x3 x4 x5 x6 : ℝ}
    ...
    (hd4 : 0 < deltaX4f_p11 x1 x2 x3 x4 x5 x6) :
    (dihXf_p11 x1 x2 x3 x4 x5 x6 < u ↔
      Real.sqrt (4 * x1 * deltaXf_p11 x1 x2 x3 x4 x5 x6)
        < Real.tan u * deltaX4f_p11 x1 x2 x3 x4 x5 x6) := by
  sorry
...
theorem lindihpi_lt_y_small_p11 {b y1 y2 y3 y4 y5 y6 : ℝ} (hy1 : 0 < y1)
    (hb : 0 < b) (hd : 0 ≤ deltaY_p11 y1 y2 y3 y4 y5 y6)
    (hd4 : 0 < delta4Y_p11 y1 y2 y3 y4 y5 y6) :
    (4 * x1DeltaY_p11 y1 y2 y3 y4 y5 y6
        < b * delta4SquaredY_p11 y1 y2 y3 y4 y5 y6 ↔
      dihY_p11 y1 y2 y3 y4 y5 y6 < Real.arctan (Real.sqrt b)) := by
  sorry

/-- lp_details.hl:1348 `dih_y_imp_delta_y_nz`. -/
theorem dih_y_imp_delta_y_nz_p11 {y1 y2 y3 y4 y5 y6 : ℝ} (hy1 : 0 < y1)
    (hpi : dihY_p11 y1 y2 y3 y4 y5 y6 < Real.pi)
    (hd4 : delta4Y_p11 y1 y2 y3 y4 y5 y6 < 0) :
      ...
      ineq_p11 (dartStd4_p11 y1 y2 y3 y4 y5 y6 y7 y8 y9)
        (tauq_p11 y1 y2 y3 y4 y5 y6 y7 y8 y9 < 0.206 ∨
          ...
          tauq_p11 y1 y2 y3 y4 y5 y6 y7 y8 y9
              + 0.7573 * dihY_p11 y1 y2 y3 y4 y5 y6 - 1.433 > 0.0 ∨
          deltaY_p11 y1 y2 y3 y4 y5 y6 < 0) := by
  sorry
...
theorem quad_3862621143_revised_wlog_p11
    ...
    ∀ y0 y1 y2 y3 y5 y6 y7 y8 y9 : ℝ,
      ineq_p11
        ...
        (taum_p11 y1 y2 y7 y9 y0 y6 + taum_p11 y1 y3 y7 y8 y0 y5 < 0.206 ∨
          taum_p11 y1 y2 y7 y9 y0 y6 + taum_p11 y1 y3 y7 y8 y0 y5
              - 0.453 * (dihY_p11 y1 y2 y7 y9 y0 y6 +
                dihY_p11 y1 y3 y7 y8 y0 y5) + 0.777 > 0.0 ∨
          deltaY_p11 y1 y2 y7 y9 y0 y6 < 0 ∨
          deltaY_p11 y1 y3 y7 y8 y0 y5 < 0 ∨
          dihY_p11 y1 y3 y7 y8 y0 y5 < dihY_p11 y1 y2 y7 y9 y0 y6) := by
  sorry
        ...
        (taum_p11 y1 y2 y7 y9 y0 y6 + taum_p11 y1 y3 y7 y8 y0 y5 < 0.206 ∨
          taum_p11 y1 y2 y7 y9 y0 y6 + taum_p11 y1 y3 y7 y8 y0 y5
              - 0.453 * (dihY_p11 y1 y2 y7 y9 y0 y6 +
                dihY_p11 y1 y3 y7 y8 y0 y5) + 0.777 > 0.0 ∨
          deltaY_p11 y1 y2 y7 y9 y0 y6 < 0 ∨
          deltaY_p11 y1 y3 y7 y8 y0 y5 < 0) := by
  sorry
...
noncomputable def mkSimplex1_p11 (v0 v1 v2 : V3) (x1 x2 x3 x4 x5 x6 : ℝ) : V3 :=
  let uinv := 1 / upsX_p11 x1 x2 x6
  let d := deltaXf_p11 x1 x2 x3 x4 x5 x6
  let d5 := deltaX5f_p11 x1 x2 x3 x4 x5 x6
  let d4 := deltaX4f_p11 x1 x2 x3 x4 x5 x6
  let vcross : V3 :=
...
def PQCSXWG1_concl_p11 : Prop :=
  ∀ v0 v1 v2 v3 : V3, ∀ x1 x2 x3 x4 x5 x6 : ℝ,
    0 < x1 ∧ 0 < x2 ∧ 0 < x3 ∧ 0 < x4 ∧ 0 < x5 ∧ 0 < x6 ∧
      ...
      0 < deltaXf_p11 x1 x2 x3 x4 x5 x6 ∧
...
theorem CONTINUOUS_MK_SIMPLEX_WITHINREAL_p11
    ...
    (hups : upsX_p11 (x1 a) (x2 a) (x6 a) ≠ 0)
    (hd : 0 < deltaXf_p11 (x1 a) (x2 a) (x3 a) (x4 a) (x5 a) (x6 a))
