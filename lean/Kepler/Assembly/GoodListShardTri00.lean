/-
  Kepler/Assembly/GoodListShardTri00 — good_list 量产分片（P6-C，机器生成）。

  生成器：`lean/scripts/gen_goodlist_shards.py`（slice=1000）；请勿手改，
  重新生成会覆盖。覆盖 TriData 第 [0, 9) 图（全类 9 图）。
...
/-- 分片闭合：`TriData` 上 `goodListB` 全真。 -/
theorem goodListTri00 : (TriData).all goodListB = true := by
  native_decide

#print axioms goodListTri00
