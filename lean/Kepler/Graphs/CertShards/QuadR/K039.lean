/- Shard group [195, 200) for seed Quad (p = 1).
...
/-- Shards [195, 200): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quadr_shard_195 : (List.range' 195 5).all (fun j =>
...
theorem quadr_shardE_195 : ∀ j ∈ List.range' 195 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_195) j hj)⟩
