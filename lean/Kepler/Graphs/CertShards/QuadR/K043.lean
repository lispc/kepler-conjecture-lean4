/- Shard group [215, 220) for seed Quad (p = 1).
...
/-- Shards [215, 220): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quadr_shard_215 : (List.range' 215 5).all (fun j =>
...
theorem quadr_shardE_215 : ∀ j ∈ List.range' 215 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_215) j hj)⟩
