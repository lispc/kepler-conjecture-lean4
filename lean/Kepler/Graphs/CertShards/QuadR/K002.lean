/- Shard group [10, 15) for seed Quad (p = 1).
...
/-- Shards [10, 15): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quadr_shard_10 : (List.range' 10 5).all (fun j =>
...
theorem quadr_shardE_10 : ∀ j ∈ List.range' 10 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_10) j hj)⟩
