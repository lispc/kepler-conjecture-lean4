/- Shard group [550, 555) for seed Quad (p = 1).
...
/-- Shards [550, 555): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quadr_shard_550 : (List.range' 550 5).all (fun j =>
...
theorem quadr_shardE_550 : ∀ j ∈ List.range' 550 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_550) j hj)⟩
