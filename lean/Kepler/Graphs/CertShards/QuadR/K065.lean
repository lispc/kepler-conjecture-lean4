/- Shard group [525, 530) for seed Quad (p = 1).
...
/-- Shards [525, 530): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quadr_shard_525 : (List.range' 525 5).all (fun j =>
...
theorem quadr_shardE_525 : ∀ j ∈ List.range' 525 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_525) j hj)⟩
