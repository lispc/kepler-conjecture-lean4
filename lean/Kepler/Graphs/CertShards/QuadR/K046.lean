/- Shard group [230, 235) for seed Quad (p = 1).
...
/-- Shards [230, 235): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quadr_shard_230 : (List.range' 230 5).all (fun j =>
...
theorem quadr_shardE_230 : ∀ j ∈ List.range' 230 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_230) j hj)⟩
