/- Shard group [15, 20) for seed Quad (p = 1).
...
/-- Shards [15, 20): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quadr_shard_15 : (List.range' 15 5).all (fun j =>
...
theorem quadr_shardE_15 : ∀ j ∈ List.range' 15 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_15) j hj)⟩
