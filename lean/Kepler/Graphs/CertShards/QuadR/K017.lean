/- Shard group [85, 90) for seed Quad (p = 1).
...
theorem quadr_shard_85 : (List.range' 85 5).all (fun j =>
...
theorem quadr_shardE_85 : ∀ j ∈ List.range' 85 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_85) j hj)⟩
