/- Shard group [555, 560) for seed Quad (p = 1).
...
theorem quadr_shard_555 : (List.range' 555 5).all (fun j =>
...
theorem quadr_shardE_555 : ∀ j ∈ List.range' 555 5,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp quadr_shard_555) j hj)⟩
