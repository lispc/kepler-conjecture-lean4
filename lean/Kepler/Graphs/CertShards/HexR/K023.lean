/- Shard group [315, 320) for seed Hex (p = 3).
...
/-- Shards [315, 320): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_315 : (List.range' 315 5).all (fun j =>
...
theorem hexr_shardE_315 : ∀ j ∈ List.range' 315 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_315) j hj)⟩
