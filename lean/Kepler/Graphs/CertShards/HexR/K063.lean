/- Shard group [515, 520) for seed Hex (p = 3).
...
theorem hexr_shard_515 : (List.range' 515 5).all (fun j =>
...
theorem hexr_shardE_515 : ∀ j ∈ List.range' 515 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_515) j hj)⟩
