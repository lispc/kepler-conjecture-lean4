/- Shard group [410, 415) for seed Hex (p = 3).
...
/-- Shards [410, 415): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_410 : (List.range' 410 5).all (fun j =>
...
theorem hexr_shardE_410 : ∀ j ∈ List.range' 410 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_410) j hj)⟩
