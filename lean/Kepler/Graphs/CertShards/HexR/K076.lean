/- Shard group [580, 585) for seed Hex (p = 3).
...
theorem hexr_shard_580 : (List.range' 580 5).all (fun j =>
...
theorem hexr_shardE_580 : ∀ j ∈ List.range' 580 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_580) j hj)⟩
