/- Shard group [565, 570) for seed Hex (p = 3).
...
theorem hexr_shard_565 : (List.range' 565 5).all (fun j =>
...
theorem hexr_shardE_565 : ∀ j ∈ List.range' 565 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_565) j hj)⟩
