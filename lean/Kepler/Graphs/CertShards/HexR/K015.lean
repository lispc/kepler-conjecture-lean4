/- Shard group [75, 80) for seed Hex (p = 3).
...
theorem hexr_shard_75 : (List.range' 75 5).all (fun j =>
...
theorem hexr_shardE_75 : ∀ j ∈ List.range' 75 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_75) j hj)⟩
