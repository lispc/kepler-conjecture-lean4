/- Shard group [0, 5) for seed Hex (p = 3).
...
theorem hexr_shard_0 : (List.range' 0 5).all (fun j =>
...
theorem hexr_shardE_0 : ∀ j ∈ List.range' 0 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_0) j hj)⟩
