/- Shard group [65, 70) for seed Hex (p = 3).
...
theorem hexr_shard_65 : (List.range' 65 5).all (fun j =>
...
theorem hexr_shardE_65 : ∀ j ∈ List.range' 65 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_65) j hj)⟩
