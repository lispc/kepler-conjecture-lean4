/- Shard group [4690, 4695) for seed Hex (p = 3).
...
/-- Shards [4690, 4695): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_4690 : (List.range' 4690 5).all (fun j =>
...
theorem hexr_shardE_4690 : ∀ j ∈ List.range' 4690 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_4690) j hj)⟩
