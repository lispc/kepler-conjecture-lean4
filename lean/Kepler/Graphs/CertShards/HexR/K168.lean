/- Shard group [3540, 3545) for seed Hex (p = 3).
...
/-- Shards [3540, 3545): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_3540 : (List.range' 3540 5).all (fun j =>
...
theorem hexr_shardE_3540 : ∀ j ∈ List.range' 3540 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_3540) j hj)⟩
