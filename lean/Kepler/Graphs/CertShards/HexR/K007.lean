/- Shard group [35, 40) for seed Hex (p = 3).
...
theorem hexr_shard_35 : (List.range' 35 5).all (fun j =>
...
theorem hexr_shardE_35 : ∀ j ∈ List.range' 35 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_35) j hj)⟩
