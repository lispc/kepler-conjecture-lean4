/- Shard group [1740, 1745) for seed Hex (p = 3).
...
/-- Shards [1740, 1745): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_1740 : (List.range' 1740 5).all (fun j =>
...
theorem hexr_shardE_1740 : ∀ j ∈ List.range' 1740 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_1740) j hj)⟩
