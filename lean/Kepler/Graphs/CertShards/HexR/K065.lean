/- Shard group [525, 530) for seed Hex (p = 3).
...
theorem hexr_shard_525 : (List.range' 525 5).all (fun j =>
...
theorem hexr_shardE_525 : ∀ j ∈ List.range' 525 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_525) j hj)⟩
