/- Shard group [4315, 4320) for seed Hex (p = 3).
...
/-- Shards [4315, 4320): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_4315 : (List.range' 4315 5).all (fun j =>
...
theorem hexr_shardE_4315 : ∀ j ∈ List.range' 4315 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_4315) j hj)⟩
