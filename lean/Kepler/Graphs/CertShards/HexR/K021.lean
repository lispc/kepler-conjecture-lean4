/- Shard group [305, 310) for seed Hex (p = 3).
...
/-- Shards [305, 310): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_305 : (List.range' 305 5).all (fun j =>
...
theorem hexr_shardE_305 : ∀ j ∈ List.range' 305 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_305) j hj)⟩
