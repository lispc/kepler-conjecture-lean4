/- Shard group [2610, 2615) for seed Hex (p = 3).
...
/-- Shards [2610, 2615): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_2610 : (List.range' 2610 5).all (fun j =>
...
theorem hexr_shardE_2610 : ∀ j ∈ List.range' 2610 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_2610) j hj)⟩
