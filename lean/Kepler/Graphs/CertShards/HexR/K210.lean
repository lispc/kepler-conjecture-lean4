/- Shard group [4650, 4655) for seed Hex (p = 3).
...
/-- Shards [4650, 4655): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_4650 : (List.range' 4650 5).all (fun j =>
...
theorem hexr_shardE_4650 : ∀ j ∈ List.range' 4650 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_4650) j hj)⟩
