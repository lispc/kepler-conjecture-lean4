/- Shard group [495, 500) for seed Hex (p = 3).
...
/-- Shards [495, 500): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_495 : (List.range' 495 5).all (fun j =>
...
theorem hexr_shardE_495 : ∀ j ∈ List.range' 495 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_495) j hj)⟩
