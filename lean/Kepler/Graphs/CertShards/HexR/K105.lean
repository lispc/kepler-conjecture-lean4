/- Shard group [1725, 1730) for seed Hex (p = 3).
...
/-- Shards [1725, 1730): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_1725 : (List.range' 1725 5).all (fun j =>
...
theorem hexr_shardE_1725 : ∀ j ∈ List.range' 1725 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_1725) j hj)⟩
