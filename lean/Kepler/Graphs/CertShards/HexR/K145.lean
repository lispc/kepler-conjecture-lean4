/- Shard group [3325, 3330) for seed Hex (p = 3).
...
/-- Shards [3325, 3330): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_3325 : (List.range' 3325 5).all (fun j =>
...
theorem hexr_shardE_3325 : ∀ j ∈ List.range' 3325 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_3325) j hj)⟩
