/- Shard group [2660, 2665) for seed Hex (p = 3).
...
/-- Shards [2660, 2665): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_2660 : (List.range' 2660 5).all (fun j =>
...
theorem hexr_shardE_2660 : ∀ j ∈ List.range' 2660 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_2660) j hj)⟩
