/- Shard group [4395, 4400) for seed Hex (p = 3).
...
/-- Shards [4395, 4400): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hexr_shard_4395 : (List.range' 4395 5).all (fun j =>
...
theorem hexr_shardE_4395 : ∀ j ∈ List.range' 4395 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_4395) j hj)⟩
