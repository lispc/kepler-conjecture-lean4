/- Shard group [45, 50) for seed Hex (p = 3).
...
theorem hexr_shard_45 : (List.range' 45 5).all (fun j =>
...
theorem hexr_shardE_45 : ∀ j ∈ List.range' 45 5,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp hexr_shard_45) j hj)⟩
