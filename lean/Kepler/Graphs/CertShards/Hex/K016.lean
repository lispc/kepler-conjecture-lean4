/- Shard group [1600, 1700) for seed Hex (p = 3).
...
/-- Shards [1600, 1700): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_1600 : (List.range' 1600 100).all (fun j =>
...
theorem hex_shardE_1600 : ∀ j ∈ List.range' 1600 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_1600) j hj)⟩
