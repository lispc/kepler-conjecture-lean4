/- Shard group [4000, 4100) for seed Hex (p = 3).
...
/-- Shards [4000, 4100): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_4000 : (List.range' 4000 100).all (fun j =>
...
theorem hex_shardE_4000 : ∀ j ∈ List.range' 4000 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_4000) j hj)⟩
