/- Shard group [2300, 2400) for seed Hex (p = 3).
...
/-- Shards [2300, 2400): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_2300 : (List.range' 2300 100).all (fun j =>
...
theorem hex_shardE_2300 : ∀ j ∈ List.range' 2300 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_2300) j hj)⟩
