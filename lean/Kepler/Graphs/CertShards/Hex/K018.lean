/- Shard group [1800, 1900) for seed Hex (p = 3).
...
/-- Shards [1800, 1900): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_1800 : (List.range' 1800 100).all (fun j =>
...
theorem hex_shardE_1800 : ∀ j ∈ List.range' 1800 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_1800) j hj)⟩
