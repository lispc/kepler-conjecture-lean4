/- Shard group [7200, 7300) for seed Hex (p = 3).
...
/-- Shards [7200, 7300): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_7200 : (List.range' 7200 100).all (fun j =>
...
theorem hex_shardE_7200 : ∀ j ∈ List.range' 7200 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_7200) j hj)⟩
