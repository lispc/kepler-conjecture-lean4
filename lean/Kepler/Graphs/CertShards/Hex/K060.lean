/- Shard group [6000, 6100) for seed Hex (p = 3).
...
/-- Shards [6000, 6100): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_6000 : (List.range' 6000 100).all (fun j =>
...
theorem hex_shardE_6000 : ∀ j ∈ List.range' 6000 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_6000) j hj)⟩
