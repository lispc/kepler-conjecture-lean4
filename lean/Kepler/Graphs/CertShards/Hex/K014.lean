/- Shard group [1400, 1500) for seed Hex (p = 3).
...
theorem hex_shard_1400 : (List.range' 1400 100).all (fun j =>
...
theorem hex_shardE_1400 : ∀ j ∈ List.range' 1400 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_1400) j hj)⟩
