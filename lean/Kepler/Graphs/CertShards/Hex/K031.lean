/- Shard group [3100, 3200) for seed Hex (p = 3).
...
/-- Shards [3100, 3200): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_3100 : (List.range' 3100 100).all (fun j =>
...
theorem hex_shardE_3100 : ∀ j ∈ List.range' 3100 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_3100) j hj)⟩
