/- Shard group [3000, 3100) for seed Hex (p = 3).
...
/-- Shards [3000, 3100): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_3000 : (List.range' 3000 100).all (fun j =>
...
theorem hex_shardE_3000 : ∀ j ∈ List.range' 3000 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_3000) j hj)⟩
