/- Shard group [3600, 3700) for seed Hex (p = 3).
...
/-- Shards [3600, 3700): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_3600 : (List.range' 3600 100).all (fun j =>
...
theorem hex_shardE_3600 : ∀ j ∈ List.range' 3600 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_3600) j hj)⟩
