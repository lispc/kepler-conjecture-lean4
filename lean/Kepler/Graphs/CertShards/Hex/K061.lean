/- Shard group [6100, 6200) for seed Hex (p = 3).
...
/-- Shards [6100, 6200): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_6100 : (List.range' 6100 100).all (fun j =>
...
theorem hex_shardE_6100 : ∀ j ∈ List.range' 6100 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_6100) j hj)⟩
