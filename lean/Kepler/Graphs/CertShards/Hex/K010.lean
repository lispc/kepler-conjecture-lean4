/- Shard group [1000, 1100) for seed Hex (p = 3).
...
theorem hex_shard_1000 : (List.range' 1000 100).all (fun j =>
...
theorem hex_shardE_1000 : ∀ j ∈ List.range' 1000 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_1000) j hj)⟩
