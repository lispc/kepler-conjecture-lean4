/- Shard group [1100, 1200) for seed Hex (p = 3).
...
theorem hex_shard_1100 : (List.range' 1100 100).all (fun j =>
...
theorem hex_shardE_1100 : ∀ j ∈ List.range' 1100 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_1100) j hj)⟩
