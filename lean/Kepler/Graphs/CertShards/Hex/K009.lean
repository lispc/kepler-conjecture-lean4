/- Shard group [900, 1000) for seed Hex (p = 3).
...
theorem hex_shard_900 : (List.range' 900 100).all (fun j =>
...
theorem hex_shardE_900 : ∀ j ∈ List.range' 900 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_900) j hj)⟩
