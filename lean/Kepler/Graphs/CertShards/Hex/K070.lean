/- Shard group [7000, 7100) for seed Hex (p = 3).
...
/-- Shards [7000, 7100): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_7000 : (List.range' 7000 100).all (fun j =>
...
theorem hex_shardE_7000 : ∀ j ∈ List.range' 7000 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_7000) j hj)⟩
