/- Shard group [2400, 2500) for seed Hex (p = 3).
...
/-- Shards [2400, 2500): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_2400 : (List.range' 2400 100).all (fun j =>
...
theorem hex_shardE_2400 : ∀ j ∈ List.range' 2400 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_2400) j hj)⟩
