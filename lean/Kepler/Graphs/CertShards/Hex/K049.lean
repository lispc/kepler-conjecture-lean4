/- Shard group [4900, 5000) for seed Hex (p = 3).
...
/-- Shards [4900, 5000): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_4900 : (List.range' 4900 100).all (fun j =>
...
theorem hex_shardE_4900 : ∀ j ∈ List.range' 4900 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_4900) j hj)⟩
