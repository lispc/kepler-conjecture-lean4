/- Shard group [600, 700) for seed Hex (p = 3).
...
theorem hex_shard_600 : (List.range' 600 100).all (fun j =>
...
theorem hex_shardE_600 : ∀ j ∈ List.range' 600 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_600) j hj)⟩
