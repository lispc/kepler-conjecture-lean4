/- Shard group [4500, 4600) for seed Hex (p = 3).
...
/-- Shards [4500, 4600): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_4500 : (List.range' 4500 100).all (fun j =>
...
theorem hex_shardE_4500 : ∀ j ∈ List.range' 4500 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_4500) j hj)⟩
