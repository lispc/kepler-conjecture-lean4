/- Shard group [1200, 1300) for seed Hex (p = 3).
...
theorem hex_shard_1200 : (List.range' 1200 100).all (fun j =>
...
theorem hex_shardE_1200 : ∀ j ∈ List.range' 1200 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_1200) j hj)⟩
