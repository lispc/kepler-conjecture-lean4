/- Shard group [5300, 5400) for seed Hex (p = 3).
...
/-- Shards [5300, 5400): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_5300 : (List.range' 5300 100).all (fun j =>
...
theorem hex_shardE_5300 : ∀ j ∈ List.range' 5300 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_5300) j hj)⟩
