/- Shard group [7100, 7200) for seed Hex (p = 3).
...
/-- Shards [7100, 7200): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_7100 : (List.range' 7100 100).all (fun j =>
...
theorem hex_shardE_7100 : ∀ j ∈ List.range' 7100 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_7100) j hj)⟩
