/- Shard group [6900, 7000) for seed Hex (p = 3).
...
/-- Shards [6900, 7000): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_6900 : (List.range' 6900 100).all (fun j =>
...
theorem hex_shardE_6900 : ∀ j ∈ List.range' 6900 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_6900) j hj)⟩
