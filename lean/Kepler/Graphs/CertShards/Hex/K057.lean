/- Shard group [5700, 5800) for seed Hex (p = 3).
...
/-- Shards [5700, 5800): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem hex_shard_5700 : (List.range' 5700 100).all (fun j =>
...
theorem hex_shardE_5700 : ∀ j ∈ List.range' 5700 100,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp hex_shard_5700) j hj)⟩
