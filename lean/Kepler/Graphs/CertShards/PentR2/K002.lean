/- Shard group [72, 73) for seed Pent (p = 2).
...
/-- Shards [72, 73): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pentr2_shard_72 : (List.range' 72 1).all (fun j =>
...
theorem pentr2_shardE_72 : ∀ j ∈ List.range' 72 1,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨300000000, of_decide_eq_true ((List.all_eq_true.mp pentr2_shard_72) j hj)⟩
