/- Shard group [129, 130) for seed Pent (p = 2).
...
/-- Shards [129, 130): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pentr2_shard_129 : (List.range' 129 1).all (fun j =>
...
theorem pentr2_shardE_129 : ∀ j ∈ List.range' 129 1,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨300000000, of_decide_eq_true ((List.all_eq_true.mp pentr2_shard_129) j hj)⟩
