/- Shard group [2, 3) for seed Pent (p = 2).
...
theorem pentr3_shard_2 : (List.range' 2 1).all (fun j =>
...
theorem pentr3_shardE_2 : ∀ j ∈ List.range' 2 1,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨300000000, of_decide_eq_true ((List.all_eq_true.mp pentr3_shard_2) j hj)⟩
