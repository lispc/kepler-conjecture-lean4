/- Shard group [9, 10) for seed Pent (p = 2).
...
/-- Shards [9, 10): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pentr3_shard_9 : (List.range' 9 1).all (fun j =>
...
theorem pentr3_shardE_9 : ∀ j ∈ List.range' 9 1,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨300000000, of_decide_eq_true ((List.all_eq_true.mp pentr3_shard_9) j hj)⟩
