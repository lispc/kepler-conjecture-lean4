/- Shard group [6650, 6700) for seed Pent (p = 2).
...
/-- Shards [6650, 6700): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_6650 : (List.range' 6650 50).all (fun j =>
...
theorem pent_shardE_6650 : ∀ j ∈ List.range' 6650 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_6650) j hj)⟩
