/- Shard group [6450, 6500) for seed Pent (p = 2).
...
/-- Shards [6450, 6500): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_6450 : (List.range' 6450 50).all (fun j =>
...
theorem pent_shardE_6450 : ∀ j ∈ List.range' 6450 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_6450) j hj)⟩
