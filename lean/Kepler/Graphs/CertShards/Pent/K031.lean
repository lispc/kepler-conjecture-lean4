/- Shard group [1550, 1600) for seed Pent (p = 2).
...
/-- Shards [1550, 1600): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_1550 : (List.range' 1550 50).all (fun j =>
...
theorem pent_shardE_1550 : ∀ j ∈ List.range' 1550 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_1550) j hj)⟩
