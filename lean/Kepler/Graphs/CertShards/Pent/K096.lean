/- Shard group [4800, 4850) for seed Pent (p = 2).
...
/-- Shards [4800, 4850): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_4800 : (List.range' 4800 50).all (fun j =>
...
theorem pent_shardE_4800 : ∀ j ∈ List.range' 4800 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_4800) j hj)⟩
