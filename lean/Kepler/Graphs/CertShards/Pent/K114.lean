/- Shard group [5700, 5750) for seed Pent (p = 2).
...
/-- Shards [5700, 5750): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_5700 : (List.range' 5700 50).all (fun j =>
...
theorem pent_shardE_5700 : ∀ j ∈ List.range' 5700 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_5700) j hj)⟩
