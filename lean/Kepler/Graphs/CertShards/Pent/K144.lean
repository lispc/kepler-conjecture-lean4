/- Shard group [7200, 7250) for seed Pent (p = 2).
...
/-- Shards [7200, 7250): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_7200 : (List.range' 7200 50).all (fun j =>
...
theorem pent_shardE_7200 : ∀ j ∈ List.range' 7200 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_7200) j hj)⟩
