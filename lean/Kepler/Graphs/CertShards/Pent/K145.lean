/- Shard group [7250, 7263) for seed Pent (p = 2).
...
/-- Shards [7250, 7263): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_7250 : (List.range' 7250 13).all (fun j =>
...
theorem pent_shardE_7250 : ∀ j ∈ List.range' 7250 13,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_7250) j hj)⟩
