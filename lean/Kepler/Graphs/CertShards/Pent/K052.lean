/- Shard group [2600, 2650) for seed Pent (p = 2).
...
/-- Shards [2600, 2650): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_2600 : (List.range' 2600 50).all (fun j =>
...
theorem pent_shardE_2600 : ∀ j ∈ List.range' 2600 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_2600) j hj)⟩
