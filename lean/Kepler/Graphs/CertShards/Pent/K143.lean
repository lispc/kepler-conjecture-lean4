/- Shard group [7150, 7200) for seed Pent (p = 2).
...
/-- Shards [7150, 7200): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_7150 : (List.range' 7150 50).all (fun j =>
...
theorem pent_shardE_7150 : ∀ j ∈ List.range' 7150 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_7150) j hj)⟩
