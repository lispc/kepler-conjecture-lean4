/- Shard group [1450, 1500) for seed Pent (p = 2).
...
/-- Shards [1450, 1500): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_1450 : (List.range' 1450 50).all (fun j =>
...
theorem pent_shardE_1450 : ∀ j ∈ List.range' 1450 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_1450) j hj)⟩
