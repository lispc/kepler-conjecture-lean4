/- Shard group [5850, 5900) for seed Pent (p = 2).
...
/-- Shards [5850, 5900): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_5850 : (List.range' 5850 50).all (fun j =>
...
theorem pent_shardE_5850 : ∀ j ∈ List.range' 5850 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_5850) j hj)⟩
