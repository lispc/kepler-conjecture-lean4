/- Shard group [5650, 5700) for seed Pent (p = 2).
...
/-- Shards [5650, 5700): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pent_shard_5650 : (List.range' 5650 50).all (fun j =>
...
theorem pent_shardE_5650 : ∀ j ∈ List.range' 5650 50,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pent_shard_5650) j hj)⟩
