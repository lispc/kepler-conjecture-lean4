/- Shard group [1, 2) for seed Hex (p = 3).
...
theorem hexr2_shard_1 : (List.range' 1 1).all (fun j =>
...
theorem hexr2_shardE_1 : ∀ j ∈ List.range' 1 1,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨400000000, of_decide_eq_true ((List.all_eq_true.mp hexr2_shard_1) j hj)⟩
