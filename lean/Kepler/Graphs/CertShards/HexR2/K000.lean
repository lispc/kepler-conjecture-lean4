/- Shard group [0, 1) for seed Hex (p = 3).
...
theorem hexr2_shard_0 : (List.range' 0 1).all (fun j =>
...
theorem hexr2_shardE_0 : ∀ j ∈ List.range' 0 1,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨400000000, of_decide_eq_true ((List.all_eq_true.mp hexr2_shard_0) j hj)⟩
