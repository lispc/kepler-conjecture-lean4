/- Shard group [3, 4) for seed Hex (p = 3).
...
theorem hexr2_shard_3 : (List.range' 3 1).all (fun j =>
...
theorem hexr2_shardE_3 : ∀ j ∈ List.range' 3 1,
    ∃ fuel, loop (next_tame 3) (checkFinal (buildBuckets HexData)) fuel
      [HexFrontier[j]!] = some true := fun j hj =>
  ⟨400000000, of_decide_eq_true ((List.all_eq_true.mp hexr2_shard_3) j hj)⟩
