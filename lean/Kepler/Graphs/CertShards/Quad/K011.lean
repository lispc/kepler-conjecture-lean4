/- Shard group [1100, 1200) for seed Quad (p = 1).
...
/-- Shards [1100, 1200): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quad_shard_1100 : (List.range' 1100 100).all (fun j =>
...
theorem quad_shardE_1100 : ∀ j ∈ List.range' 1100 100,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp quad_shard_1100) j hj)⟩
