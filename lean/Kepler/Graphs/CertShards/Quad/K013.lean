/- Shard group [1300, 1400) for seed Quad (p = 1).
...
/-- Shards [1300, 1400): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quad_shard_1300 : (List.range' 1300 100).all (fun j =>
...
theorem quad_shardE_1300 : ∀ j ∈ List.range' 1300 100,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp quad_shard_1300) j hj)⟩
