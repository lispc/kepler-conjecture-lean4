/- Shard group [800, 900) for seed Quad (p = 1).
...
/-- Shards [800, 900): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem quad_shard_800 : (List.range' 800 100).all (fun j =>
...
theorem quad_shardE_800 : ∀ j ∈ List.range' 800 100,
    ∃ fuel, loop (next_tame 1) (checkFinal (buildBuckets QuadData)) fuel
      [QuadFrontier[j]!] = some true := fun j hj =>
  ⟨4000000, of_decide_eq_true ((List.all_eq_true.mp quad_shard_800) j hj)⟩
