/- Shard group [120, 130) for seed Pent (p = 2).
...
/-- Shards [120, 130): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pentr_shard_120 : (List.range' 120 10).all (fun j =>
...
theorem pentr_shardE_120 : ∀ j ∈ List.range' 120 10,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pentr_shard_120) j hj)⟩
