/- Shard group [60, 70) for seed Pent (p = 2).
...
/-- Shards [60, 70): worklist closure from each
frontier root certifies `checkFinal` (`loop_some_true`). -/
theorem pentr_shard_60 : (List.range' 60 10).all (fun j =>
...
theorem pentr_shardE_60 : ∀ j ∈ List.range' 60 10,
    ∃ fuel, loop (next_tame 2) (checkFinal (buildBuckets PentData)) fuel
      [PentFrontier[j]!] = some true := fun j hj =>
  ⟨50000000, of_decide_eq_true ((List.all_eq_true.mp pentr_shard_60) j hj)⟩
