...
| 9 | `apex_flat_hll` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3236 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 10 | `apex_flat_l` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3701 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 11 | `apex_std3_hll` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3343 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 12 | `apex_std3_lhh` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3903 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
...
| 35 | `dart4_diag3` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:2866 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 36 | `dart4_diag3_b` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:2591 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 37 | `dartX` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:2828 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 38 | `dartY` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:2847 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 39 | `dart_mll_n` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3373 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 40 | `dart_mll_w` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3363 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
...
| 43 | `dart_std3_big_200_218` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3256 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 44 | `dart_std3_lw` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3552 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 45 | `dart_std3_mini` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3191 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 46 | `dart_std3_small` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:3046 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
| 47 | `dart_std4` | unsupported | D | - |  | auto:reference/flyspeck/text_formalization/nonlinear/ineq.hl:1863 | define_dart 盒域谓词：emit_rpn.py:832 dart_constraint_ast 把它展开成 (lo,var,hi) 数值盒（别名链递归，实测三元组 6/9 条），domain_raw 只进盒约束不进 prog（translate_record:947），==> 前件由 strip_spine:899 剥离丢弃——不进 IExpr，豁免移植 |
...
| 153 | `x1_delta_x` | resolved | B | x1DeltaX (lean/Kepler/Text/IneqClosureDefs.lean:636) | 6 | reference/flyspeck/text_formalization/general/sphere.hl:840 |  |
